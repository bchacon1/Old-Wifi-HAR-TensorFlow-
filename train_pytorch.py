...
all_features_npy_path = os.path.join("input_files", "all_features_full.npy")
all_labels_npy_path = os.path.join("input_files", "all_labels_full.npy")
...
if LOAD_FROM_NPY and os.path.exists(all_features_npy_path) and os.path.exists(all_labels_npy_path):
    ...
    elif all_features_full.shape[1] != window_size:
        raise ValueError(
            f"Unexpected feature timestep dimension {all_features_full.shape[1]}"
        )
    # Drop the deprecated "NoActivity" column if present
    ...
    def load_csv_pair(label):
        ...
        # The CSVs generated by `cross_vali_data_convert_merge.py` already
        # exclude "NoActivity" samples. Older CSVs may still include the first
        # column as an unused indicator, so drop it if present.
        if labels_csv.shape[1] == 8:
            labels_csv = labels_csv[:, 1:]

        # Determine how many timesteps are encoded in the CSV.
        num_timesteps = features_csv.shape[1] // n_input
        features_csv = features_csv.reshape(-1, num_timesteps, n_input)
        if num_timesteps == raw_window_size:
            # Older CSVs store 1000 timesteps; downsample to 500 to match the
            # TensorFlow preprocessing.
            features_csv = features_csv[:, ::2, :]
        elif num_timesteps != window_size:
            raise ValueError(
                f"Unexpected number of timesteps {num_timesteps} in {xx_file_path}"
            )
